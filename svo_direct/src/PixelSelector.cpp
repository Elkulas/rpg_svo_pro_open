...
{
  int th = hist[0]*below+0.5f; // 最低的像素个数,百分之五十的格子数目  32*32*0.5 = 496
  for(int i=0;i<90;i++) // 90? 这么随便...., 因为最高也就是50
  {
    th -= hist[i+1];  // NOTICE: 梯度值为0-i的所有像素个数占 below %
    if(th<0) return i;
  }
  return 90;
}
    ...
    for(int x=0;x<w32;x++) {
      ...
      for (int j = 0; j < 32; j++)
        for (int i = 0; i < 32; i++) {
          ...
          // NOTICE: 也就是说,这边因为了解到最大的梯度的情况也就是255的根号即50,所以设置了50这个数字
      ...
      // std::cout << "Hello " << x + y * w32 << " now is " << ths[x + y * w32] << std::endl;

        //***************************
    //  int color = 256* ths[x + y * w32]/(50+setting_minGradHistAdd);
    //  drawMat(gradents, cv::Point2i(y * 32, x * 32), cv::Scalar(color,color,color), 15);
    //  imshow("test", gradents);
    //  cvWaitKey(1);

    }// 遍历格子, 目的获得各个格子中的阈值,存在ths中
...
//      cout << x << ":" << y << endl;
      ...
      // std::cout << "Hello " << x + y * w32 << " now is " << thsSmoothed[x + y * w32] << std::endl;
...
//[ ***step 5*** ] 现在提取的还是多, 随机删除一些点
  ...
  if (plot) {
    ...
    MinimalImageB3 img2(w, h);
    MinimalImageB3 img4(w, h);

    for (int i = 0; i < w * h; i++) {
      float c = fh->dI[i][0] * 0.7; // 像素值
      if (c > 255) c = 255;
      img.at(i) = Vec3b_(c, c, c);
      img2.at(i) = Vec3b_(c, c, c);
      img4.at(i) = Vec3b_(c, c, c);
    }
    ...
    for (int y = 0; y < h; y++)
      for (int x = 0; x < w; x++) {
        ...
        if (map_out[i] == 1)
        {
          img.setPixelCirc(x, y, Vec3b_(0, 255, 0));
        }
        else if (map_out[i] == 2)
        {
          img.setPixelCirc(x, y, Vec3b_(255, 0, 0));
          img2.setPixelCirc(x, y, Vec3b_(255, 0, 0));
        }
        else if (map_out[i] == 4)
        {
          img.setPixelCirc(x, y, Vec3b_(0, 0, 255));
          img4.setPixelCirc(x, y, Vec3b_(0, 0, 255));
        }
      }
//    IOWrap::displayImage("Selector Pixels", &img);

    cv::Mat Pixel_Selected = cv::Mat(img.h, img.w, CV_8UC3);
    cv::Mat Pixel_Selected2 = cv::Mat(img.h, img.w, CV_8UC3);
    cv::Mat Pixel_Selected4 = cv::Mat(img.h, img.w, CV_8UC3);
//  toCvMat(fh->absSquaredGrad[0], gradents);

    memcpy(Pixel_Selected.data, img.data->data(), img.h * img.w*3 );
    memcpy(Pixel_Selected2.data, img2.data->data(), img.h * img.w*3 );
    memcpy(Pixel_Selected4.data, img4.data->data(), img.h * img.w*3 );
    imshow("Pixel_Selected", Pixel_Selected);
    imshow("Pixel_Selected2", Pixel_Selected2);
    imshow("Pixel_Selected4", Pixel_Selected4);
    cv::waitKey(0);
